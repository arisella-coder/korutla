...
import { Vendor, Consumer } from '../models/users/usersModel.js';
...
export async function register(req, res) {
  try {
    const { name, email, username, password, role, storeName, phone } = req.body;
    console.log('Registering user:', req.body);

    if (!['vendor', 'consumer'].includes(role)) {
    ...
    transporter.sendMail(mailOptions, (error, info) => {
      ...
      res.status(201).json({
        message: 'Registration initiated. OTP sent to your email. Please verify OTP to complete registration.',
      });
    });
  } catch (err) {
